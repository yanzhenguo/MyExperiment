...
def get_input():
    with open(root_dir + "temp/yahoo/extract_data/texts.pkl", 'rb') as f:
        texts = pickle.load(f)
    ...
    x_train = x[:num_train]
    x_test = x[num_train:]
    y = np.load(root_dir + 'temp/yahoo/extract_data/label.npy')
...
if __name__ == '__main__':
    ...
    model = get_model()
    model.fit([x_train], y_train, batch_size=4096, epochs=10, shuffle=True,
