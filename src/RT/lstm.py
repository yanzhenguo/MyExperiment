...
from keras.layers import Input, Embedding, AveragePooling1D, Dense, GlobalMaxPooling1D, GlobalAveragePooling1D, Dropout,\
    BatchNormalization, RepeatVector, SpatialDropout1D, GRU
...
def get_input():
    ...
    x=x[indice]
    x_reverse=x_reverse[indice]
    y=y[indice]


    return x, x_reverse, y, embedding_matrix
...
def get_model(embedding_matrix):
    ...
    x=GRU(300)(embedding1)

    input_2 = Input(shape=(max_len,))
    embedding2 = Embedding(num_words, 300, weights=[embedding_matrix],
                           trainable=False)(input_2)
    y= GRU(300)(embedding2)
