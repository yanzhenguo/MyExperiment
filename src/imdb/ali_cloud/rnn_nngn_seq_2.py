...
def get_input():
    ...
    x_seq= np.zeros((50000, (max_len - 1) * 2), dtype=np.int)
    for i in range(50000):
        for j in range(max_len - 1):
            x_seq[i, j * 2] = x[i, j]
            x_seq[i, j * 2 + 1] = x[i][j + 1] + num_words
...
def get_model(embedding_matrix):
    ...
    input_3=Input(((max_len - 1) * 2,))
    embedding_3 = Embedding(num_words*2, embedding_dimension)(input_3)
    z = AveragePooling1D(pool_size=2, strides=2, padding='valid')(embedding_3)
