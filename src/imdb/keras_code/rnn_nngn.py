...
def get_input():
    ...
    for seq in sequences:
        sequences_reverse.append(list(reversed(seq)))

    x = pad_sequences(sequences, maxlen=max_len)
    x_reverse=pad_sequences(sequences, maxlen=max_len)

    word_index = tokenizer.word_index
    embeddings_index = {}
    wordX = np.load('/home/yan/my_datasets/glove/embedding.300d.npy')
    f = open('/home/yan/my_datasets/glove/words.pkl', 'rb')
...
def get_model(embedding_matrix):
    ...
    z=AveragePooling1D(pool_size=3, strides=1, padding='valid')(embedding_3)
...
if __name__=='__main__':
    ...
    model.fit(result[0:2],result[4],batch_size=128,epochs=10,validation_data=(result[2:4],result[5]))